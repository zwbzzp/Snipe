...
def stop_vm(vmid):
    try:
        ...
        # Add Resume Desktop Task
        stop_desktop_task = DesktopTask()
        stop_desktop_task.state = 'PENDING'
        stop_desktop_task.action = TaskAction.STOP
        stop_desktop_task.stage_chain = json.dumps(
            ['STOP', 'WAIT'])
        stop_desktop_task.stage = 'STOP'
        stop_desktop_task.context = json.dumps({
            'course_name': desktop.course.name if desktop.course else None,
            'serial': desktop.name.split('_')[-1] if desktop.course else None,
            'desktop': desktop.id,
            'wait_state': 'SHUTOFF',
        ...
        db.session.add(stop_desktop_task)
        db.session.commit()

        celery_tasks.run_desktoptask(task_id=stop_desktop_task.id)
        return True
    except:
        LOG.error("Stop VM Failed: %s" % vmid)
        return False
