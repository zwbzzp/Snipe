...
def image_list():
    ...
    try:
        ...
        LOG.exception("Get Image List Failed")
...
def update_image():
    ...
    if image_edit_form.validate_on_submit():
        try:
            ...
            OpenstackImageService.update_image(
                image_id=image_edit_form.imageid.data,
                name=image_edit_form.imagename.data)
            ...
            LOG.exception("Update Image Faile ")
    return jsonify(**result)
        ...
        try:
            ...
            if error_list != []:
                ...
                for imageid in ids:
                    ...
                    LOG.info("Delete Image Successfully: %s " % imageid)
                result['status'] = 'success'
        except Exception as ex:
            LOG.exception("Delete Image Fail")
            result['status'] = 'fail'
        return jsonify(**result)
    ...
    try:
        ...
        try:
            ...
            for network in networks:
                network_list.append({'id':network.id,'name':network.label})
        form = LaunchInstanceForm()
        return render_template('image/image_generator.html',
        ...
        LOG.exception("Get Image Generator Info Fail")
    return render_template('image/image_generator.html')
        ...
        if launch_instance_form.validate_on_submit():
            ...
            if Desktop.query.filter_by(name=instance_name).first():
                ...
                if valid:
                    ...
                    LOG.info("成功生成实例: %s" % instance_name)
                else:
                    result['status'] = "fail"
    except Exception as ex:
        LOG.exception("Launch Instance Failed")
        result['status'] = 'fail'
    return jsonify(**result)
...
def delete_instance():
    ...
    try:
        ...
        if not desktop and instance:
            ...
            LOG.info("成功删除实例: %s" % instance.name)
        ...
        LOG.exception("Delete Instance Failed")
        result['status'] = 'fail'
    return jsonify(**result)
    ...
    try:
        ...
        if existedFlag:
            ...
            if utils.create_snapshot(vmid, name,current_user.id,
                ...
                LOG.info("成功创建快照: %s_%s" % (vmid, name))
                result['status'] = 'success'
            else:
                result['status'] = "fail"
    except Exception as ex:
        LOG.exception("Create_snapshot Failed")
        result['status'] = 'fail'
    return jsonify(**result)
...
def get_instance_console(vmid):
    try:
        ...
        LOG.exception("Get instance console Failed")
        instance_console = None
    return render_template("image/instance_console.html",instance_console=instance_console)
