...
def users(role_name):
    ...
    if role:
        ...
        if role.name == 'Administrator' and admin_type_perm.can():
            html_template = 'account/admin.html'
        elif role.name == 'Teacher' and teacher_type_perm.can():
            html_template = 'account/teacher.html'
        elif role.name == 'Student' and student_type_perm.can():
            html_template = 'account/student.html'
        else:
            abort(403)
...
def create_user():
    ...
    if form.validate_on_submit():
        ...
        LOG.info("创建用户: %s" % user_model.username)
        return jsonify({'status': 'success',
                        'data': user})
    LOG.debug("用户信息不合法，创建用户失败")
    return jsonify({
        'status': 'fail',
                ...
                for samba_account in samba_accounts:
                    ...
                    if samba_server:
                        try:
                            ...
                            LOG.exception("Delete %s's samba account which on [%s] fail" % (user, samba_server.ip))
...
def update_user_status(id):
    ...
    if user:
        ...
        if not user.is_super_administrator():
            ...
            LOG.info("%s user %s" % ("Active" if status else "Deactive", user.fullname))
            return jsonify({'status': 'success',
                            'data': {'id':user.id, 'username':user.username, 'is_active':user.is_active} })
    LOG.debug("%s an non-exists user with id %s" % ("Active" if status else "Deactive", id))
...
def update_user(id):
    ...
    if form.validate_on_submit():
        ...
        if user_model:
            ...
            LOG.info("User %s is updated" % user_model.fullname)
            user = {
                'id': user_model.id,
                'username':user_model.username,
                'fullname': user_model.fullname,
            ...
            LOG.debug("User %s is not exist" % user_model.fullname)
            return jsonify({'status': 'fail',
                            'data': 'user not exist'})
    LOG.debug("Invalid user information, update failed")
    return jsonify({
        'data': {'form_errors': form.errors},
        'status': 'fail',
    })
    ...
    if file is not None:
        try:
            if file.content_length > (5 * 1024 * 1024):
                ...
                LOG.debug("The file is too large")
            ...
            if file_type == "xls" or file_type == "xlsx":
                ...
                LOG.info("批量导入用户列表,成功%s个,失败%s个" % (user_count, fail_count))
                if fail_count > 0:
                    json['status'] = 'part_success'
                    LOG.debug("部分用户未成功导入")
                else:
                    json['status'] = 'success'
                    LOG.debug("全部用户批量导入成功")
            else:
                LOG.debug("导入文件格式不正确，导入失败")
                json["status"] = "fail"
                json["data"]["error_msg"] = "type error"
        except Exception as e:
            LOG.exception("批量导入用户列表失败")
            json["status"] = "fail"
            json["data"]["error_msg"] = "exception"
    else:
        LOG.warning("用户列表文件没有提交")
        json["status"] = "fail"
...
def get_user_info():
    ...
    try:
        ...
        LOG.exception("Get User Info Failed")
        result['status'] = "fail"
    ...
    try:
        ...
        if administrator and administrator.verify_password(admin_pwd):
            ...
            if user and user.verify_password(user_pwd):
                LOG.info("成功认证用户: %s" % user.username)
                ua_logger.info(current_user, "成功认证用户: %s" % user.username)
                result["status"] = "success"
            else:
                LOG.debug("认证用户 %s 失败: 用户不存在，或用户密码认证错误" % user.username)
                result["status"] = "user_fail"
        else:
            LOG.debug("认证管理员 %s 失败: 用户不存在，或用户密码认证错误" % administrator.username)
            result["status"] = "admin_fail"
    except Exception as ex:
        LOG.exception("Auth User Failed")
        result["status"] = "fail"
    return jsonify(**result)
...
def modify_password():
    ...
    for user_id in users:
        ...
        if user:
            ...
            db.session.add(user)
            db.session.commit()
            LOG.info("User %s changed password" % user.fullname)
            ua_logger.info(current_user, "重置了用户%s密码" % user.username)
            result_json['data']['success_list'].append(user.username)
        else:
            LOG.debug("Trying to change passwd for an non-exists user %s" % user.fullname)
            result_json['data']['fail_list'].append(user_id)
    if len(result_json['data']['fail_list']) > 0:
        LOG.debug("Fail to change passwd for some users in selected_user list")
    ...
    if form.validate_on_submit():
        ...
        if User.verify_password(user_model,form.oldpasswd.data):
            if user_model:
                ...
                db.session.add(user_model)
                db.session.commit()
                LOG.info("User %s changed password" % current_user.fullname)
                ua_logger.info(current_user, "用户%s重置了密码" % current_user.fullname)
                result_json['status'] = 'success'
            else:
                LOG.error("User %s fail to change password" % current_user.fullname)
                result_json['status'] = 'fail'
                result_json['errorinfo'] = '密码修改过错中出现错误'
        else:
            LOG.error("User %s input an incorrect old password" % current_user.fullname)
            result_json['status'] = 'fail'
            result_json['errorinfo'] = '请输入正确的原用户密码'
    else:
        LOG.error("Invalid password information, update fail")
        result_json['status'] = 'fail'
        result_json['errorinfo'] = '密码修改过错中出现错误'
    return jsonify(result_json)
