...
def desktop_to_json(desktop, default_protocol='rdp', policy=None):
    if desktop:
        # TODO: default_connection_type should achieve from desktop
        json_desktop = {
            'id': str(desktop.id),
            'name': desktop.name,
            'status': desktop.desktop_state,
        ...
        # add policy to the response
        if policy:
            json_desktop['policy']['enable_usb'] = policy['enable_usb']
            json_desktop['policy']['enable_clipboard'] = policy['enable_clipboard']
            json_desktop['policy']['enable_audio'] = policy['enable_audio']
