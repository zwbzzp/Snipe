...
def choice_of_policy():
    from ..models import Policy
    policies = Policy.query.all()
    return [(policy.id, policy.name) for policy in policies]
...
def default_of_policy():
    from ..models import Parameter, Policy
    default_policy_name = Parameter.query.filter_by(name='default_policy').first().value
    if default_policy_name is None:
        return 1
    result = Policy.query.filter_by(name = default_policy_name).first().id
    return result
...
class CourseForm(Form):
    ...
    policy_id = SelectField('外设策略', validators=[data_required()], coerce=int, choices=[])
    ...
    def __init__(self, *args, **kwargs):
        ...
        self.policy_id.choices = choice_of_policy()
        self.policy_id.default = default_of_policy()
